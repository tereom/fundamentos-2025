...
```{r binsmoother-animation-cuad, echo=FALSE, warning=FALSE, message=FALSE, fig.align='center'}
library(gganimate)
library(broom)
ventas <- readr::read_csv(here::here("data", "ventas_semanal.csv"))

if(!fs::file_exists(here::here("images", "02_loess-spans-cuad.gif"))){
  # spans a considerar
  spans <- c(.05, .10, .5, 1)
  ...
  fits <- crossing(ventas, span = spans) |>
    nest_by(span) |>
    mutate(
      mod = list(loess(sales.kg ~ period, data = data, degree = 2,
                       span = span)),
      data_mod = list(augment(mod))
    )
  ...
  loess_spans <- ggplot(dat, aes(period, sales.kg)) +
    ...
    transition_manual(center) +
    geom_smooth(aes(group = center, frame = center, weight = weight),
                method = "lm", formula = y ~ poly(x, degree = 2), se = FALSE) +
    geom_vline(aes(xintercept = center, frame = center), lty = 2) +
    geom_point(shape = 1, data = ventas, alpha = .25) +
    geom_line(aes(y = .fitted, frame = period, cumulative = TRUE),
              data = unnest(fits, cols = data_mod), color = "red") +
    facet_wrap(~span, ncol = 1) +
    ggtitle("Loess cuadrático")
...
```{r, fig.width=3.4, fig.height=3.2, fig.align='center'}
tricubo <- function(x) {
...
g_2 <- ggplot(datos_dia, aes(x = fecha, y = n)) +
  ...
  geom_smooth(method = "loess", span = 0.075, method.args = list(degree = 2), se = FALSE) + xlab("") +
...
```{r, fig.width=10, echo = FALSE,  out.width = '90%', fig.align= 'center', cache=TRUE, fig.asp = 0.55, message=FALSE}
datos_da <- datos_dia |>
...
```{r, echo = FALSE, fig.height=2, echo = FALSE,  out.width = '90%', fig.align= 'center', cache=TRUE, fig.asp = 0.35, message=FALSE}
