...
Introducimos los algoritmos en términos
...
2. Aceptamos or rechazamos el salto:
  - Si $\alpha = \frac{p(\theta^*|x)}{p(\theta^{(i)}|x)} \geq 1$, aceptamos el salto
  y ponemos $\theta^{(i+1)}=\theta^*$. Regresamos a 1 para la siguiente
  iteración $i\leftarrow i + 1.$
  - Si $\alpha = \frac{p(\theta^*|x)}{p(\theta^{(i)}|x)} < 1$, entonces aceptamos
...
Abajo implementamos el algoritmo con un salto de tipo normal. En la versión
de Metroplois
...
veremos en la clase) y aún así no alcanza convergencia la reparametrización 
puede ayudar [guía](https://mc-stan.org/docs/2_21/stan-users-guide/reparameterization-section.html).
