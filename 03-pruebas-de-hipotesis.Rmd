...
te <- tea |> dplyr::select(how, price, sugar)
```

```{r, echo = FALSE}
te_azucar <- tea |> 
  dplyr::select(how, sugar) |>
...
prop_azucar <- te_azucar |> 
  ...
  filter(sugar == "sugar") |>
  dplyr::select(how, prop_azucar = prop, n) |>
...
```{r, fig.width = 6, fig.height = 3, echo = FALSE}
g_1 <- ggplot(tibble(diferencia = valores_ref), aes(sample = diferencia)) +
  geom_qq(distribution = stats::qunif)  +
  labs(x = "f", y = "Cociente media intercuartil", 
       subtitle = "Distribución nula o de referencia") +
  geom_hline(yintercept = dif_obs, colour = "red") +
  annotate("text", x = 0.3, y = dif_obs - 0.05, label = "diferencia observada", colour = "red")
g_2 <- ggplot(tibble(diferencia = valores_ref), aes(x = diferencia)) +
  geom_histogram(binwidth = 0.04) +
  coord_flip() + xlab("") + labs(subtitle = " ") +
  geom_vline(xintercept = dif_obs, colour = "red") +
  annotate("text", x = dif_obs, y = N_rep * .15, label = cuantil_obs,vjust = 0,
           colour = "red")
g_1 + g_2
```
